...
for nazwa_pliku in ddd/nazwy-*; do
    while read -r nazwa; do
        echo "0" > "ddd/zasoby/$nazwa"
    done < "$nazwa_pliku"
done

for nazwa_pliku in ddd/nazwy-*; do
    while read -r nazwa; do
	wartosc=$(< "ddd/zasoby/$nazwa")
	((wartosc++))
	echo "$wartosc" > "ddd/zasoby/$nazwa"
    done < "$nazwa_pliku"
done


for plik in ddd/zasoby/*; do
